      ...
      <div className="relative z-10 max-w-7xl mx-auto p-8">
        <div className="flex justify-between items-center mb-4">
          <div className="flex gap-3">
            <button onClick={() => navigate('/timeline')} className="px-4 py-2 bg-purple-900/50 hover:bg-purple-800 rounded-lg text-white text-sm font-semibold transition-all flex items-center gap-2 border border-purple-600/50">
              <ArrowLeft className="w-4 h-4" />
              My Projects
            </button>
            {isAdmin && (
              <button onClick={() => navigate('/video-manager')} className="px-4 py-2 bg-purple-600 hover:bg-purple-700 rounded-lg text-white text-sm font-semibold transition-all flex items-center gap-2 border border-purple-500/50">
          ...
          <label className="px-4 py-2 bg-blue-600 hover:bg-blue-700 rounded-lg text-white text-sm font-semibold transition-all flex items-center gap-2 cursor-pointer">
            ...
            <input type="file" multiple accept="image/*,video/*,audio/*" onChange={handleFileUpload} className="hidden" disabled={isUploading} />
          </label>
        </div>
        <div className="text-center mb-12">
          <h1 className="text-7xl font-black text-transparent bg-gradient-to-r from-purple-400 via-purple-500 to-purple-600 bg-clip-text mb-4">EDITOR SUITE</h1>
          <p className="text-2xl text-gray-300 font-light">Professional-Grade Video Editing Platform</p>
        </div>
        <div className="bg-gradient-to-b from-purple-900/30 to-transparent backdrop-blur-xl rounded-2xl p-8 border-2 border-purple-500/50 mb-12 shadow-2xl shadow-purple-500/20">
          ...
          <div className="mt-4 bg-purple-950/30 border border-purple-700/30 rounded-lg p-4">
            <p className="text-gray-300 text-sm text-center"><span className="text-purple-400 font-semibold">💡 Tip:</span> The prompt automatically adjusts to your selected duration ({movieDuration} minutes). Choose a template or write custom instructions for best results.</p>
          </div>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-16">
          {editorFeatures.map((feature, index) => (
            <div key={index} className="bg-gradient-to-b from-white/5 to-transparent backdrop-blur-xl rounded-2xl p-8 border border-purple-700/50 hover:border-purple-500 transition-all transform hover:scale-105">
        ...
        <div className="bg-gradient-to-b from-purple-900/20 to-transparent backdrop-blur-xl rounded-2xl p-12 border border-purple-700/50 text-center mb-12">
          ...
          <p className="text-xl text-gray-300 mb-8">Upload your media files and jump into our complete suite of professional editing tools!</p>
          <div className="flex gap-4 justify-center">
            <button onClick={() => navigate('/media-library')} className="px-10 py-4 bg-gradient-to-r from-blue-600 to-blue-700 hover:from-blue-700 hover:to-blue-800 text-white text-xl font-bold rounded-xl transition-all transform hover:scale-105 shadow-lg shadow-blue-500/50">Media Library</button>
            <button onClick={() => navigate('/export')} className="px-10 py-4 bg-gradient-to-r from-purple-600 to-purple-700 hover:from-purple-700 hover:to-purple-800 text-white text-xl font-bold rounded-xl transition-all transform hover:scale-105 shadow-lg shadow-purple-500/50">Timeline Editor</button>
          </div>
          <p className="text-sm text-gray-400 mt-4">Upload media, edit on timeline, and export your masterpiece</p>
        </div>
        <div className="bg-gradient-to-b from-purple-900/20 to-transparent backdrop-blur-xl rounded-2xl p-8 border border-purple-700/50 mb-8">
          ...
          <div className="space-y-6">
            <div className="text-center mb-6">
              <div className="text-7xl font-black text-transparent bg-gradient-to-r from-purple-400 to-blue-400 bg-clip-text mb-2">{movieDuration}</div>
              <div className="text-xl text-purple-400 font-semibold">MINUTES</div>
              <div className="text-sm text-gray-400 mt-2">({Math.floor(movieDuration / 60)}h {movieDuration % 60}m)</div>
            </div>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-6">
              <button onClick={() => setMovieDuration(30)} className={`px-6 py-4 rounded-xl font-bold transition-all ${movieDuration === 30 ? 'bg-gradient-to-r from-purple-600 to-blue-600 text-white shadow-lg shadow-purple-500/50' : 'bg-purple-950/50 text-gray-300 hover:bg-purple-900/50'}`}>30 min</button>
              <button onClick={() => setMovieDuration(60)} className={`px-6 py-4 rounded-xl font-bold transition-all ${movieDuration === 60 ? 'bg-gradient-to-r from-purple-600 to-blue-600 text-white shadow-lg shadow-purple-500/50' : 'bg-purple-950/50 text-gray-300 hover:bg-purple-900/50'}`}>60 min</button>
              <button onClick={() => setMovieDuration(90)} className={`px-6 py-4 rounded-xl font-bold transition-all ${movieDuration === 90 ? 'bg-gradient-to-r from-purple-600 to-blue-600 text-white shadow-lg shadow-purple-500/50' : 'bg-purple-950/50 text-gray-300 hover:bg-purple-900/50'}`}>90 min</button>
              <button onClick={() => setMovieDuration(120)} className={`px-6 py-4 rounded-xl font-bold transition-all ${movieDuration === 120 ? 'bg-gradient-to-r from-purple-600 to-blue-600 text-white shadow-lg shadow-purple-500/50' : 'bg-purple-950/50 text-gray-300 hover:bg-purple-900/50'}`}>120 min</button>
            </div>
            <div className="space-y-3">
              ...
              <input type="range" min="1" max="180" value={movieDuration} onChange={(e) => setMovieDuration(Number(e.target.value))} className="w-full h-4 bg-purple-950/50 rounded-lg appearance-none cursor-pointer slider" style={{background: `linear-gradient(to right, rgb(147, 51, 234) 0%, rgb(147, 51, 234) ${(movieDuration / 180) * 100}%, rgb(88, 28, 135) ${(movieDuration / 180) * 100}%, rgb(88, 28, 135) 100%)`}} />
            ...
            <div className="bg-purple-950/30 border border-purple-700/30 rounded-lg p-4">
              <p className="text-gray-300 text-sm text-center"><span className="text-purple-400 font-semibold">Tip:</span> Select a preset duration or drag the slider to customize up to 3 hours. Your timeline and AI prompts will adjust automatically.</p>
            </div>
          </div>
        </div>
        <div className="flex justify-between">
          <button onClick={() => navigate('/timeline')} className="flex items-center gap-2 px-6 py-3 bg-purple-900/50 hover:bg-purple-800 text-white font-bold rounded-lg transition-all border border-purple-600/50">
            <ArrowLeft className="w-5 h-5" />
            Back
          </button>
          <button onClick={() => navigate('/export')} className="flex items-center gap-2 px-6 py-3 bg-gradient-to-r from-purple-600 to-purple-700 hover:from-purple-700 hover:to-purple-800 text-white font-bold rounded-lg transition-all">
            Next
